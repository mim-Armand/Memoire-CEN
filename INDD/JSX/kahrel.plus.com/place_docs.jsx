...
function get_story ()
	...
	try {doc.textPreferences.limitToMasterTextFrames = false}
		catch(_){}
...
//~ 	var prefs = [], temp = Folder (app.scriptPreferences.scriptsFolder.parent.parent + "/Word Import Presets/").getFiles('*.smp');
//~ 	for (var i = 0; i < temp.length; i++)
//~ 		prefs.push (temp[i].name);
...
//~ 		var presetPanel = main_group.add ('panel {_: StaticText {text: "Presets:"}, orientation: "row"}')
//~ 			var presets = presetPanel.add ('dropdownlist', undefined, prefs);
//~ 				presets.preferredSize.width += 10
//~ 				presets.selection = 0;
		

		var b = w.add ('group {orientation: "column"}');
			var up = b.add ('button {text: "Up"}');
			var down = b.add ('button {text: "Down"}');
			var rem = b.add ('button {text: "Remove"}');
			var select_all = b.add ('button {text: "Select all"}');
			var ok = b.add ('button {text: "OK"}');
			var cancel = b.add ('button {text: "Cancel"}');
...
function find_files (dir)
	{
	if (dir.slice(-1) != "/") dir += "/";
	var array = Folder (dir).getFiles ("*.doc");
	array = array.concat (Folder (dir).getFiles ("*.docx"));
	array = array.concat (Folder (dir).getFiles ("*.rtf"));
	if (array.length > 0)
		{
		for (var i = 0; i < array.length; i++)
			{
			array[i] = decodeURI (array[i].fullName);
...
function read_history (f /* file object */)
    {
    if (f.exists)
        {
        f.open ("r"); var temp = f.read (); f.close ();
        if (temp.length > 0)
			{
			return temp.split (/[\r\n]/);
			}

...
/*
	22 March 2012: Separated adding destinations and references.
...
function dynamic_endnotes ()
	...
	var mess = create_counter ();
	convert_endnotes (app.documents[0], mess, styles);
	mess.close(); mess = null;
	}

function convert_endnotes (doc, mess, styles)
	{
	// delete phantom applications of the character style
	// (i.e. the char style applied to white space)
	mess.txt.text = "Fixing references..."; mess.show();
...
//~	 	var par_style_numbered = par_style;
	    ...
	    crossref_notes (doc, styles.character, par_style_numbered, cr_format, mess);
		
	    // remove the manual numbers in the endnotes
	    remove_old_numbers (doc, par_style_numbered);
	    }
	
	mess.txt.text = "Updating numbers..."; mess.show();
	reset_numbers (doc, par_style_numbered)
	}


function crossref_notes (doc, reference_style, note_style, cr_format, mess)
	{
	...
	for (var i = notes.length-1; i > -1; i--)
	    ...
	    endnote_links[i] = doc.paragraphDestinations.add (notes[i].insertionPoints[0]);
	    }
...
function reset_numbers (doc, par_style_numbered)
	{
	app.findGrepPreferences = null;
	app.findGrepPreferences.appliedParagraphStyle = par_style_numbered;
...
function references_match_notes (doc, c_style, p_style)
	...
	if (references != notes)
		...
		if (notes.length < 2)
			m += "\r\r(You probably need to convert the numbered paragraphs\r in the notes to manually numbered paragraphs.)";
	    errorM (m);
	    }
	return true
	}

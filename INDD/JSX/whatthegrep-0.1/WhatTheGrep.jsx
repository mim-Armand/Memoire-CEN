...
// [[.ss.]]
			...
			/*	//	Allow Suppressed special character -- an error, here!
					...
					if (expr.substr(0,2) == "\\Q")
					{
						...
						/*	switch (str)
							{
								...
								default:
									if (ignoreCase && str.match(/[A-Za-z]/))
										addLn (dest, str+"\tLiteral text р"+str+"с (case ignored)", nestGroup+1);
									else
										addLn (dest, str+"\tLiteral text р"+str+"с", nestGroup+1);
							} */
						...
						//	colorcoded.push ([expr.substr(0,2), colorCode.Escape]);
							...
							if (lastChar == 9)
								...
								lastChar = 'the character р'+lastChar+'с';
				...
				} else
				{
					...
					expr = expr.substr(2);
					expr = canRepeat (dest, expr, 'the character р'+expr.substr(1,1)+'с');
				}
				break;
			...
			//	addLn (dest, "Remaining: {"+expr+"}");
					expr = canRepeat (dest, expr, 'the code for р'+str+'с');
					break;
				}
					...
					*/
							...
							if (ignoreWhitespace)
							{
								...
								//	colorcoded.push ([str, colorCode.Text]);
									if (str == ' ')
										...
										expr = canRepeat (dest, expr, 'the character р'+str+'с', str);
									break;
								}
								...
								/*	if (ignoreCase && spaceless.match(/[A-Za-z\u00c0-\u00d6\u00d8-\u00f6\u00f8-\u01bf\u01c4-\u0249\u0386-\u513\u1e00-\u1eff\u1f00-\u1fcc]/))
										addLn (dest, spaceless+"\tLiteral text р"+spaceless+"с (ignoring case)", nestGroup);
									else
										addLn (dest, spaceless+"\tLiteral text р"+spaceless+"рс", nestGroup); */
									addLnForText (dest, str, nestGroup, ignoreCase);
								}
								str = spaceless;
							} else
						...
						//	addLn (dest, "["+expr+"] remaining");
								...
								/*	if (ignoreCase && str.match(/[A-Za-z\u00c0-\u00d6\u00d8-\u00f6\u00f8-\u01bf\u01c4-\u0249\u0386-\u513\u1e00-\u1eff\u1f00-\u1fcc]/))
										addLn (dest, str+"\tLiteral text р"+str+"с (ignoring case)", nestGroup);
									else
										addLn (dest, str+"\tLiteral text р"+str+"с", nestGroup); */
									addLnForText (dest, str, nestGroup, ignoreCase);
								}
							}
							...
							{
								colorcoded.push ([str, colorCode.Text]);
							//	expr = canRepeat (dest, expr, 'the character р'+str.substr(str.length-1)+'с');
							}
						} else
						{
			...
			default:
				...
				else
				{
					...
					/*	if (block[1] == '-')
						...
						if (block[0].length == 1)
							addLn (dest, block[0]+'\tThe character р'+block[0]+'с', nestGroup);
						else
...
function addLnForText (dest, text, nestGroup, ignoreCase)
{
	...
	switch (text)
	{
		...
		default:
			if (text.length == 1)
				addLn (dest, text+'\tLiteral character р'+text+'с'+ignoring, nestGroup);
			else
				addLn (dest, text+'\tLiteral text р'+text+'с'+ignoring, nestGroup);
	}
}
